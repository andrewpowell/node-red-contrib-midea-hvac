...
module.exports = async (username, password) => {
    ...
    client.on('message', async (message, info) => {
        ...
        if (message[0] === 0x83 && message[1] === 0x70) {
            message = message.subarray(8, message.length - 16);
            newEncryptionVersion = true;
        }

        if (message[0] === 0x5A && message.length >= 104) {
            // Get the bytes specifying the Id and reverse them
            const id = parseInt(message.subarray(20, 26).toString('hex').match(/../g).reverse().join(''), 16);
            const data = decrypt(message.subarray(40, msg.length - 16));
    ...
    return await new Promise((resolve, reject) =>
        ...
        msg.payload.appliances = appliances;
        node.send(msg);
        node.done();
    });
};
